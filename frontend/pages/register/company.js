...
const NEED_TYPES = [
  { value: "franchise", label: "Franchisees", color: "bg-franchise" },
  { value: "dealer", label: "Dealers", color: "bg-dealer" },
  { value: "associate", label: "Associates", color: "bg-associate" },
  { value: "job", label: "Employees", color: "bg-job" },
];
...
export default function RegisterCompany() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      const data = await registerUser({ ...form, user_type: "company" });
      localStorage.setItem("access_token", data.access);
      localStorage.setItem("refresh_token", data.refresh);
      localStorage.setItem("pending_needs", JSON.stringify(needs));
    ...
    <main className="min-h-screen flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="bg-surface border border-rule rounded-2xl p-8">
          ...
          <p className="text-xs text-offwhite/50 mb-3">We are looking for:</p>
          <div className="mb-6 space-y-2">
            {NEED_TYPES.map((opt) => (
              <label key={opt.value} className="flex items-center gap-2.5 text-sm text-offwhite/80 cursor-pointer">
                <input type="checkbox" checked={needs.includes(opt.value)}
